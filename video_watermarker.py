...
def validate_video_path(answers, current):
    ...
    if not os.path.exists(current):
        raise ValidationError("", reason="File not found. Please enter a valid path.")
    return True
...
def validate_text_input(answers, current):
    ...
    if not line1.strip() and not line2.strip():
        raise ValidationError("", reason="At least one line of text is required.")
    return True
...
def validate_directory_path(answers, current):
    ...
    if not os.path.exists(current) or not os.path.isdir(current):
        raise ValidationError("", reason="Directory not found. Please enter a valid directory path.")
    return True
...
def add_watermark_to_video(video_path, text_line1, text_line2):
    ...
    watermark_overlay = create_watermark_overlay(
        frame_width, frame_height, text_line1, text_line2
    )
